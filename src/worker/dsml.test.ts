...
// Real DSML captured from a DeepSeek reply, lightly trimmed for readability.
const SAMPLE_TEXT = `The room's fully intact — and I can see exactly where the thread stands.

Let me close the loop first this time — reschedule before anything else:`

const SAMPLE_DSML = `<｜DSML｜tool_calls>
...
const SAMPLE_FULL = SAMPLE_TEXT + '\n\n' + SAMPLE_DSML

describe('hasDsml', () => {
  it('detects DSML in text', () => {
    expect(hasDsml(SAMPLE_FULL)).toBe(true)
  })
...
describe('findDsmlStart', () => {
  it('returns position of DSML start', () => {
    const idx = findDsmlStart(SAMPLE_FULL)
    expect(idx).toBeGreaterThan(0)
    expect(SAMPLE_FULL.slice(0, idx).trim()).toBe(SAMPLE_TEXT)
  })
...
describe('parseDsml', () => {
  it('parses real-world DSML output', () => {
    const result = parseDsml(SAMPLE_FULL)
    expect(result).not.toBeNull()
    expect(result!.cleanText).toBe(SAMPLE_TEXT)
...
describe('DsmlStreamFilter', () => {
  ...
  it('detects and suppresses DSML when it arrives in one chunk', () => {
    const filter = new DsmlStreamFilter()
    const safe = filter.feed(SAMPLE_FULL)
  ...
  it('simulates realistic streaming chunks', () => {
    const filter = new DsmlStreamFilter()
    // Simulate how DeepSeek streams: small text chunks, then DSML
    const textChunks = SAMPLE_TEXT.match(/.{1,20}/g) || []
    const dsmlChunks = SAMPLE_DSML.match(/.{1,40}/g) || []
