...
import { workspaceSandbox, resolveWorkspacePath, truncateOutput, WORKSPACE_DIR } from '../sandbox'
...
const SHARED_NOTE =
  'The workspace is a real Linux machine (Ubuntu, Python 3, Node, git, curl - full internet access). ' +
...
export const sandboxTools: Tool[] = [
  {
    def: {
      ...
      function: {
        name: 'workspace_exec',
        description:
          `Run a shell command in your workspace. ${SHARED_NOTE} ` +
    ...
    run: async (ctx, args) => {
      ...
      const sandbox = workspaceSandbox(ctx.env)
  ...
  },
  {
    def: {
      ...
      function: {
        name: 'workspace_read',
        description:
          'Read a text file from your workspace. Relative paths resolve under ' +
  ...
  },
  {
    def: {
      ...
      function: {
        name: 'workspace_edit',
        description:
          'Edit a file in your workspace by replacing an exact snippet of its current text. ' +
  ...
  },
  {
    def: {
      ...
      function: {
        ...
        description:
          'Write a text file in your workspace (creates parent directories, overwrites if it exists). ' +
